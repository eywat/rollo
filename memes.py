...
class Memes(Cog):
    ...
    def __init__(self, session: aiohttp.ClientSession, token: str):
        super().__init__()
        self._build_query = partial(
            build_query, 'https://api.tenor.com/v1', key=token)
    ...
    async def request(self, command: str, q=None, limit=50, **kwargs):
        if command == 'trending':
            cache = self.responses[command]
        elif q is None:
            raise ValueError(
                f"For {command} the query parameter q has to be set!")
        else:
            cache = self.responses[command].get(q, [])
        if cache:
            return cache.pop(0)

        LOGGER.debug('Sendind %s request with following argumens: q=%s, %s',
                     command, q, ','.join([f'{k}={v}' for k, v in kwargs.items()]))
